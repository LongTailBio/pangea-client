...
export default class HeatMap {
  ...
  update(data: HeatMapOptions) {
    ...
    const rowNameWidth = Math.min(
      maxRowNameLength * gridSize,
      options.maxAxisNameLength,
    );
    gridSize = Math.floor((canvasWidth - rowNameWidth) / (xMax + 1));
    gridSize = Math.min(
      gridSize,
      Math.floor(((canvasWidth - rowNameWidth) * 1.5) / yMax + 1),
    );
    let canvasHeight = 0;
    const axisNameSize =
      options.axisNameSize >= 0 ? options.axisNameSize : gridSize;

    // Column names
    const columnNames = this.xAxis
      .selectAll<SVGTextElement, HeatMapXAxisOptions>('.column-name')
    ...
    columnNames
      ...
      .attr('transform', (d, i) => {
        return `translate(${gridSize * (i + 0.5) + rowNameWidth}, ${itemHeight -
          3}) rotate(-90)`;
      });
    ...
    // Row names
    const rowNames = this.yAxis
      .selectAll<SVGTextElement, string>('.row-name')
      .data(data.axis.y);
    rowNames
      .enter()
      .append('text')
      ...
      .attr(
        'transform',
        (d, i) =>
          `translate(${rowNameWidth - 5}, ${canvasHeight +
            gridSize * (i + 0.5)})`,
      );
    ...
    cards
      ...
      .on('mouseover', function(d: HeatMapDatum) {
        ...
        safeTooltip
          .html(
            `(${data.axis.x[d.x].name}, ${data.axis.y[d.y]}) <b>${d.value}</b>`,
          )
    ...
    if (options.legend.name.length > 0) {
      ...
      this.legendTitle.attr('x', canvasWidth / 2).attr('y', canvasHeight);
